...
function rampChangePathWrapper(path)
{
    if (typeof ramp.changePath === "function" && path != rampCurrPath)
    {
        ramp.changePath(path);
        rampCurrPath = path;
    }
}
...
{
    //triggers destruction of ad units as no ad units are enabled for gamepage
    rampChangePathWrapper("gamepage");
}

function requestMainMenuBanner()
{
    rampChangePathWrapper("mainmenu");
}
...
{
    rampChangePathWrapper("roundend");
}
    ...
    if (firebase.auth().currentUser != null)
    {
        ...
        functionRef(null).then((response) =>
        {
            ...
            EXAMPLE RESPONSE OBJECT
            {
                ...
                "code": "AggZt2FFgZGfVi9M", // valid code
                "rewards": 0 // number of rewards earned in the past 24 hours
            }
            */

            console.log("OnFunction Complete");
            ...
            if (pwVideoCode !== undefined)
            {
                ...
                ramp.showRewardedVideo({
                    ...
                    callback: (response) =>
                    {
                        ...
                        {
                            adPlayed: true/false
                            adType: "video" | "display" | "house"
                            code: "AggZt2FFgZGfVi9M"
                            rewardUser: true/false
                            userId: "testuser"
                        }
                        */
