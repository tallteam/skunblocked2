...
(function() {
  ...
  var DEFAULTS = {
    CONTENT_ENCODING_MULTIPLE: 5.0,
    DEFAULT_CONTENT_LENGTH: 1024 * 1024 * 15,
  ...
  if (window.Proxy) {
    ...
    var calculate_pseudo_total = function(defaults, original_event) {
      var pseudo_total = 0;//1024 * 1024 * 20;
      if (!pseudo_total) {
      ...
      try {
        if (!pseudo_total) {
          console.log("AAAA SDSD original_event.target: " + original_event.target.url + " sdsdsd original_event.target.type: " + original_event.target.type);
          console.log("content length " + original_event.target.getResponseHeader('content-length'));
        ...
        if (!pseudo_total) {
          console.log("BBBBb SDSD original_event.target: " + original_event.target.url + " sdsdsd original_event.target.type: " + original_event.target.type);
          console.log("content length " + original_event.target.getResponseHeader('content-length'));
          var content_length =
          ...
          if (content_length) {
            ...
            var content_encoding =
                original_event.target.getResponseHeader(
                    'content-encoding');
            console.log("content encoding " + content_encoding);
            
            //Johnh mod: always multiply. unity loader seems to remove encoding.
      ...
      if (!pseudo_total) {
        console.log("CCCCC SDSD original_event.target: " + original_event.target.url + " sdsdsd original_event.target.type: " + original_event.target.type);
        console.log("content length " + original_event.target.getResponseHeader('content-length'));
        pseudo_total = defaults.DEFAULT_CONTENT_LENGTH;
      }

      console.log("pseudo_total " + pseudo_total);
    ...
    var onprogress = function(defaults, callback) {
      ...
      return function(event) {
        ...
        console.log("onprogress " + event.loaded + " :: " + event.total + " ::psedo total " + event.total + " :: " + event.target.getResponseHeader('content-length') + " sdsd " + event.target.getResponseHeader('content-type'));
        if (event &&
    ...
    var proxy = {
      ...
      get: function(target, name) {
        if (name == "addEventListener") {
          console.log("name + " + name + " target " + target.getResponseHeader('content-length'));
          return function() {
            if (target.xmlHTTPRequestLengthComputable[
    ...
    window.XMLHttpRequest = function(arg) {
      console.log("aaaaXMLHttpRequest " + arg);
